...
\begin{itemize}
    \item l'Accuracy, qui donne la proportion globale de prédictions correctes, mais qui doit être interprétée avec prudence compte tenu du déséquilibre des classes ;
...
\subsubsection{Matériel et écosystème logiciel}
L'ensemble du projet est développé en Python (version 3), devenu depuis plusieurs années le langage de référence de l'industrie pour les projets d'intelligence artificielle et de science des données, grâce à la richesse de son écosystème de bibliothèques dédiées et à la simplicité de sa syntaxe. Le développement est réalisé au sein d'un environnement de développement interactif de type Jupyter Notebook et Google Colab.

\subsubsection{Librairies de manipulation et de visualisation de données}
La manipulation des données repose principalement sur Pandas, qui structure les données sous forme de DataFrames et facilite leur nettoyage, leur filtrage et leur transformation, ainsi que sur NumPy, qui fournit les structures de tableaux et les opérations matricielles et vectorielles nécessaires aux calculs numériques sous-jacents à la plupart des algorithmes de machine learning.

Pour la visualisation, Matplotlib et Seaborn sont utilisés conjointement : Matplotlib pour la génération de graphiques de base, et Seaborn, construit au-dessus de Matplotlib, pour des visualisations statistiques plus élaborées (distributions, violin plots, heatmaps de corrélation), ainsi que pour la représentation des courbes d'apprentissage et des matrices de confusion utilisées dans l'évaluation des modèles.

\subsubsection{Écosystème de Machine Learning et optimisation}
La bibliothèque Scikit-Learn constitue le socle du projet pour les modèles de machine learning classiques présentés en 1.2.1 : elle fournit une implémentation standardisée et éprouvée de la Régression Linéaire, de la Régression Logistique, des SVM (SVC/SVR), des Arbres de Décision et des Random Forest, avec une interface commune facilitant leur comparaison. Elle fournit également la classe \texttt{GridSearchCV}, qui automatise la recherche par grille combinée à la validation croisée présentée en 3.2.3, permettant d'optimiser systématiquement les hyperparamètres de chacun de ces modèles. Pour l'algorithme XGBoost, la bibliothèque officielle du même nom est utilisée, celle-ci étant spécifiquement optimisée pour l'implémentation du Boosting de gradient extrême, aussi bien pour sa variante de régression (XGBRegressor) que de classification (XGBClassifier).

\subsubsection{Outils d'analyse temporelle et de Deep Learning}
Pour la composante temporelle du Module 3, la bibliothèque Statsmodels est utilisée afin d'extraire les composantes de tendance, de saisonnalité et de résidu de chaque variable physico-chimique de l'eau, via son implémentation de la décomposition STL présentée en 3.2.2. Enfin, le framework TensorFlow, à travers son interface Keras, est utilisé pour concevoir, compiler et entraîner l'architecture du réseau de neurones récurrent LSTM dédié à la prédiction de la qualité de l'eau à l'horizon d'une heure, ce framework étant aujourd'hui l'un des standards de l'industrie pour le développement de modèles de deep learning.
