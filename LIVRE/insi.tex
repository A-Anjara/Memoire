...
\hline
FOFIFA & Foibem-pirenena momba ny Fikarohana ampiharina amin'ny Fampandrosoana ny eny Ambanivohitra (Centre National de la Recherche Appliquée au Développement Rural) \\
\hline
...
Accuracy & Proportion globale de prédictions correctes d'un modèle de classification, sur l'ensemble des observations. \\[24pt]
AUC-ROC & Aire sous la courbe ROC, mesurant la capacité d'un modèle de classification à distinguer les deux classes indépendamment d'un seuil de décision fixe. \\[24pt]
Downsampling & Technique de rééquilibrage des classes consistant à réduire aléatoirement le nombre d'observations de la classe majoritaire pour l'égaliser à celui de la classe minoritaire. \\[24pt]
Encodage ordinal & Transformation d'une variable catégorielle en valeurs numériques entières croissantes, adaptée lorsque ses modalités possèdent un ordre naturel. \\[24pt]
Encodage one-hot & Transformation d'une variable catégorielle en plusieurs colonnes binaires distinctes, adaptée lorsque ses modalités n'ont pas de relation d'ordre entre elles. \\[24pt]
F1-score & Métrique combinant la Precision et le Recall en une seule valeur, utile pour comparer des modèles de classification de façon synthétique. \\[24pt]
GridSearch & Méthode d'optimisation testant systématiquement toutes les combinaisons d'une grille d'hyperparamètres candidats, généralement combinée à une validation croisée. \\[24pt]
Hyperparamètre & Paramètre d'un modèle fixé avant l'entraînement (par exemple le nombre d'arbres d'une Random Forest), par opposition aux paramètres appris automatiquement pendant l'entraînement. \\[24pt]
Kernel trick & Technique permettant à un modèle SVM de séparer des données non linéairement séparables, en calculant implicitement leur similarité dans un espace de dimension supérieure. \\[24pt]
LSTM (Long Short-Term Memory) & Architecture de réseau de neurones récurrent dotée d'un système de portes, capable de retenir des dépendances temporelles sur de longues séquences. \\[24pt]
MAE (Mean Absolute Error) & Métrique de régression correspondant à l'écart moyen absolu entre les valeurs prédites et les valeurs réelles. \\[24pt]
MAPE (Mean Absolute Percentage Error) & Métrique de régression exprimant l'erreur moyenne en pourcentage de la valeur réelle. \\[24pt]
Overfitting (surapprentissage) & Phénomène par lequel un modèle s'ajuste excessivement aux données d'entraînement, y compris à leur bruit, au détriment de sa capacité à généraliser sur de nouvelles données. \\[24pt]
Precision & Parmi les observations prédites comme positives par un modèle de classification, proportion réellement positive. \\[24pt]
Random Forest & Modèle d'ensemble combinant un grand nombre d'arbres de décision entraînés sur des échantillons et des sous-ensembles de variables différents, afin de réduire la variance et le surapprentissage. \\[24pt]
Recall (rappel) & Parmi les observations réellement positives, proportion correctement détectée par un modèle de classification. \\[24pt]
RMSE (Root Mean Squared Error) & Métrique de régression pénalisant davantage les erreurs importantes, exprimée dans la même unité que la variable prédite. \\[24pt]
Rizipisciculture & Pratique agroécologique associant l'élevage de poissons à la culture du riz sur une même parcelle inondée. \\[24pt]
$R^2$ (coefficient de détermination) & Métrique de régression indiquant la proportion de la variance de la variable cible expliquée par le modèle. \\[24pt]
Standardisation (Z-score) & Technique de mise à l'échelle transformant une variable pour obtenir une moyenne nulle et un écart-type unitaire. \\[24pt]
STL (Seasonal-Trend decomposition using Loess) & Méthode décomposant une série temporelle en une composante de tendance, une composante saisonnière et un résidu, à l'aide de la méthode de lissage Loess. \\[24pt]
SVM (Support Vector Machine) & Modèle cherchant à séparer des données en classes à l'aide d'un hyperplan maximisant la marge entre les classes. \\[24pt]
Tilapia & Espèce de poisson d'eau douce (\textit{Oreochromis niloticus}), largement élevée dans le monde pour sa croissance rapide et sa capacité d'adaptation. \\[24pt]
Turbidité & Mesure du trouble d'une eau, lié à la présence de particules en suspension, exprimée en NTU (Nephelometric Turbidity Unit). \\[24pt]
Validation croisée (Cross-validation) & Technique divisant les données d'entraînement en plusieurs sous-ensembles afin d'évaluer un modèle de façon plus fiable qu'avec un unique découpage entraînement/test. \\[24pt]
Walk-forward validation & Méthode de validation adaptée aux séries temporelles, consistant à entraîner un modèle sur une fenêtre de données passées et à l'évaluer sur la période suivante, en faisant glisser cette fenêtre dans le temps. \\[24pt]
XGBoost (Extreme Gradient Boosting) & Algorithme d'apprentissage d'ensemble construisant des arbres de décision de façon séquentielle, chaque nouvel arbre corrigeant les erreurs des arbres précédents. \\[24pt]
Zezika & Terme malgache désignant le fumier ou l'engrais organique traditionnellement utilisé pour fertiliser les rizières. \\[24pt]
\end{longtable}
...
\paragraph{Performances sur l'ensemble de test\\}
Le tableau suivant détaille les performances des cinq modèles selon les cinq métriques retenues (Accuracy, Precision, Recall, F1-score, AUC-ROC).
...
Les quatre autres modèles présentent des performances élevées et globalement proches. La Random Forest obtient le meilleur Recall (0,9253) ainsi que la meilleure Accuracy, Precision et F1-score, ce qui en fait le modèle le plus équilibré du Module 2. XGBoost, bien que très légèrement en retrait sur le Recall (0,9241), obtient en revanche le meilleur AUC-ROC (0,9656), ce qui indique une capacité globale de séparation des deux classes encore meilleure que celle de la Random Forest.
...
\section{Déploiement, discussion et perspectives}
...
Cette interface, volontairement simple, illustre la façon dont les trois modules pourraient être intégrés dans un outil unique et cohérent à
...
L'utilisation de modèles prédictifs plus avancés tels que la Random Forest, XGBoost, le SVM et les réseaux de neurones récurrents de type LSTM a permis de mieux capter ces dynamiques sous-jacentes. Les résultats obtenus montrent une amélioration significative des performances par rapport aux approches linéaires, offrant aux exploitants rizipisciculteurs des bases concrètes pour un futur outil d'aide à la décision, aussi bien pour estimer la densité de tilapia atteignable que pour surveiller et anticiper l'état de leur eau.

Ces outils permettraient non seulement de réduire les pertes liées à une mortalité évitable du cheptel piscicole, mais aussi d'encourager davantage de riziculteurs, aujourd'hui hésitants faute d'accompagnement technique, à se tourner vers la rizipisciculture, contribuant ainsi à la diversification de leurs revenus et à la sécurité alimentaire locale. Par ailleurs, ce travail met en lumière l'importance d'une démarche rigoureuse de collecte et de validation des données de terrain, condition indispensable pour bâtir un système fiable et véritablement adapté aux réalités agricoles malgaches.

En somme, ce projet offre une première contribution concrète à l'amélioration du suivi de la production rizipiscicole, et ouvre la voie à des travaux futurs portant sur la validation des modèles à partir de données de terrain réelles, l'intégration de capteurs connectés pour une collecte continue des données, ainsi que le déploiement de l'interface développée sous une forme réellement accessible aux exploitants, par exemple via une application mobile adaptée à un usage au bord de la parcelle. À l'avenir, il serait également pertinent d'explorer l'ajout de nouvelles variables environnementales ainsi que l'automatisation de la mise à jour des modèles au fil du temps, afin de renforcer la robustesse et la fiabilité du système en conditions réelles d'exploitation.
